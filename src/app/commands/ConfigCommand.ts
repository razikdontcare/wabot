...
import {getBotConfigService, getCurrentConfig, getUserRoles, log} from '../../infrastructure/config/config.js';
import {BotConfigService, StoredBotConfig} from '../../domain/services/BotConfigService.js';
...
• config reset — Reset configuration to default values`,
    ...
    async handleCommand(
        ...
        _message: proto.IWebMessageInfo
    ): Promise<void> {
        const chatId = jid;
        const senderId = user;
        ...
        try {
            switch (action) {
                case 'get':
                    ...
                    break;
                case 'set':
                    await this.handleSet(sock, chatId, args.slice(1), configService, senderId);
                    break;
                case 'reset':
                    await this.handleReset(sock, chatId, configService, senderId);
                    break;
                case 'add-admin':
                    await this.handleAddRole(sock, chatId, args.slice(1), configService, 'admin', senderId);
                    break;
                case 'remove-admin':
                    await this.handleRemoveRole(sock, chatId, args.slice(1), configService, 'admin', senderId);
                    break;
                case 'add-mod':
                    await this.handleAddRole(sock, chatId, args.slice(1), configService, 'moderator', senderId);
                    break;
                case 'remove-mod':
                    await this.handleRemoveRole(sock, chatId, args.slice(1), configService, 'moderator', senderId);
                    break;
                case 'add-vip':
                    await this.handleAddRole(sock, chatId, args.slice(1), configService, 'vip', senderId);
                    break;
                case 'remove-vip':
                    await this.handleRemoveRole(sock, chatId, args.slice(1), configService, 'vip', senderId);
                    break;
                default:
                    await this.showHelp(sock, chatId);
            }
    ...
    private async handleGet(
        ...
        chatId: string,
        args: string[],
        _configService: BotConfigService
    ): Promise<void> {
        if (args.length === 0) {
...
• RPS: ${config.emoji.rps}
    ...
    private async handleRemoveRole(
        ...
        const userJid = args[0];
        const success = await configService.removeUserFromRole(userJid, role, senderId);
