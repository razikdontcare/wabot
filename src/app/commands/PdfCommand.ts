...
• *${BotConfig.prefix}pdf cancel* — Batalkan sesi`,
  ...
  private async handleDone(
    ...
    try {
      ...
      log.info(
        `PDF created for user ${user}: ${pdfSession.images.length} pages`,
      );
    } catch (error) {
  ...
  private async handleToImage(
    ...
    if (
      ...
      await sock.sendMessage(jid, {
        text: `${config.emoji.error} Reply file PDF (mimetype application/pdf) yang mau diubah ke gambar!\n\nContoh: reply PDF terus ketik *${config.prefix}pdf toimage* atau *${config.prefix}pdf toimage 2-5*`,
      });
      return;
    }
    ...
    if (pageSpec) {
      ...
      if (!requestedPages) {
        await sock.sendMessage(jid, {
          text: `${config.emoji.error} Format range salah. Gunakan: *${config.prefix}pdf toimage 3* atau *${config.prefix}pdf toimage 2-5*`,
        });
        return;
      }
    }
    ...
    await sock.sendMessage(jid, {
      text: `${config.emoji.info} Lagi ekstrak gambar dari PDF... tunggu bentar ya! 🖼️`,
    });
    ...
    try {
      ...
      const allImages = this.extractJpegImagesFromPdf(
        pdfBuffer,
        this.MAX_PDF_PAGES_SCAN,
      );
      const totalPages = allImages.length;
      if (totalPages === 0) {
        await sock.sendMessage(jid, {
          text: `${config.emoji.error} Tidak ditemukan gambar dalam PDF ini. Mungkin bukan PDF buatan bot.`,
        });
        return;
      }
      ...
      if (requestedPages) {
        ...
        if (invalid.length) {
          await sock.sendMessage(jid, {
            text: `${config.emoji.error} Halaman ${invalid.join(", ")} tidak ada. PDF punya ${totalPages} halaman.`,
          });
          return;
        }
        imagesToSend = requestedPages.map((p) => ({
          page: p,
          data: allImages[p - 1],
        }));
        pageRangeText =
          requestedPages.length === 1
            ? `Halaman ${requestedPages[0]}`
            : `Halaman ${requestedPages[0]}-${requestedPages[requestedPages.length - 1]}`;
      } else {
        // Default: first N pages (safety)
        const limit = Math.min(this.MAX_PDF_PAGES_TO_IMAGE, totalPages);
        for (let p = 1; p <= limit; p++)
          imagesToSend.push({ page: p, data: allImages[p - 1] });
      ...
      for (const img of imagesToSend) {
        await sock.sendMessage(jid, {
          image: img.data,
          caption: `🖼️ Halaman ${img.page}/${totalPages}`,
        });
      }

      await sock.sendMessage(jid, {
        text: `${config.emoji.success} Ekstraksi selesai! ${pageRangeText}. Total halaman PDF: ${totalPages}.`,
      });
    } catch (error) {
      log.error("Error converting PDF to images:", error);
      await sock.sendMessage(jid, {
        text: `${config.emoji.error} Gagal ekstrak gambar dari PDF 😢\nCoba PDF lain atau pastikan PDF dibuat oleh bot ini.`,
      });
    }
  }
   ...
   */
  private extractJpegImagesFromPdf(
    pdfBuffer: Buffer,
    maxImages: number,
  ): Buffer[] {
    const images: Buffer[] = [];
