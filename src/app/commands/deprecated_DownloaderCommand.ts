...
type ResponseStatus<T extends Status, Extra = Record<string, never>> = {
  status: T;
} & Extra;
  ...
  async handleCommand(
    ...
    if (args.length > 0 && args[0] === "help") {
      await sock.sendMessage(jid, {
        text: `Penggunaan: ${deprecated_DownloaderCommand.commandInfo.helpText}`,
      });
      return;
    }
    ...
    for (let attempt = 1; attempt <= maxRetries + 1; attempt++) {
      try {
        ...
        if (attempt <= maxRetries) {
          ...
          log.info(
            `Request failed, retrying in ${delay}ms (attempt ${attempt}/${maxRetries + 1})`,
          );
