...
import {getUserRoles} from '../../infrastructure/config/config.js';
...
export class RegisterGroupCommand extends CommandInterface {
    ...
    async handleCommand(
        ...
        const collection: Collection = client.db().collection(GROUP_REGISTRY_COLLECTION);
        await collection.updateOne({group: jid}, {$set: {group: jid, registeredAt: new Date()}}, {upsert: true});
        await sock.sendMessage(jid, {
...
export async function getAllRegisteredGroupJids(mongoClient?: MongoClient): Promise<string[]> {
