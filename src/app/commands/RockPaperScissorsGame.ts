...
import {Session, WebSocketInfo} from '../../shared/types/types.js';
import {proto} from 'baileys';

type InputRPSChoice = 'rock' | 'paper' | 'scissors' | 'batu' | 'gunting' | 'kertas';
...
Untuk multiplayer, kedua pemain mengirim pilihan lewat DM ke bot.`,
    ...
    private readonly choices: InputRPSChoice[] = ['rock', 'paper', 'scissors', 'batu', 'gunting', 'kertas'];
    private readonly winConditions: Record<NormalizedRPSChoice, NormalizedRPSChoice> = {
        rock: 'scissors',
        paper: 'rock',
        scissors: 'paper',
    };
    ...
    async handleCommand(
        ...
        try {
            ...
            if (subCommand === 'start') {
                ...
                await this.handlePlayerMove(normalizedChoice, jid, user, sock, sessionService, isGroup);
...
${BotConfig.prefix}rps stop - Hentikan game yang sedang kamu mainkan (AI atau Multiplayer jika kamu host)`;
    ...
    private async startGame(
        ...
        if (mode === 'multiplayer') {
            ...
            const existingGame = await sessionService.getSession<RPSSession>(jid, MULTIPLAYER_SESSION_KEY);
            ...
            const existingLink = await sessionService.getSession<RPSLinkSession>(user, user);
            if (existingLink && existingLink.game === LINK_SESSION_KEY) {
                await sock.sendMessage(jid, {
                    text: `Kamu (${this.formatUserMention(user)}) sudah tergabung dalam game multiplayer lain (di grup ${
            ...
            if (!(await sessionService.setSession(jid, MULTIPLAYER_SESSION_KEY, 'rps', sessionData))) {
                await sock.sendMessage(jid, {
            ...
            if (
                ...
                await sock.sendMessage(jid, {
                    text: `Gagal menyimpan link sesi untuk ${this.formatUserMention(user)}. Pastikan bot bisa DM.`,
                });
            ...
            await sock.sendMessage(jid, {
                text: `🎮 Game Rock Paper Scissors Multiplayer dimulai oleh ${this.formatUserMention(
                    user
                )}!\n\nPemain lain, kirim pilihanmu (batu/gunting/kertas) lewat *DM* ke bot: ${BotConfig.prefix}rps [pilihan]`,
                mentions: [user],
            });
        } else {
            const existingAiSession = await sessionService.getSession<RPSSession>(jid, user);
    ...
    private async handlePlayerMove(
        ...
        if (isGroup) {
            ...
            if (aiSession && aiSession.game === 'rps' && aiSession.data.mode === 'ai') {
                await this.processAiMove(normalizedChoice, jid, user, sock, sessionService, aiSession);
            } else {
                const mpSession = await sessionService.getSession<RPSSession>(jid, MULTIPLAYER_SESSION_KEY);
                if (mpSession && mpSession.game === 'rps' && mpSession.data.mode === 'multiplayer') {
                    await sock.sendMessage(jid, {
            ...
            if (aiSession && aiSession.game === 'rps' && aiSession.data.mode === 'ai') {
                await this.processAiMove(normalizedChoice, user, user, sock, sessionService, aiSession);
                return;
            }

            // Check for a multiplayer link session (key: user, user)
            const linkSession = await sessionService.getSession<RPSLinkSession>(user, user);
            if (linkSession && linkSession.game === LINK_SESSION_KEY) {
                const groupJid = linkSession.data.groupJid;
                await this.processMultiplayerMove(normalizedChoice, groupJid, user, sock, sessionService);
            } else {
                await sock.sendMessage(jid, {
    ...
    private async processMultiplayerMove(
        ...
        const gameSession = await sessionService.getSession<RPSSession>(groupJid, MULTIPLAYER_SESSION_KEY);

        if (!gameSession || gameSession.game !== 'rps' || gameSession.data.mode !== 'multiplayer') {
        ...
        if (user === rpsSession.player1) {
            ...
            await sock.sendMessage(user, {
                text: `Kamu (P1) memilih ${this.choiceToEmoji(playerChoice)}. Menunggu player 2...`,
            });
            ...
            if (!rpsSession.player2) {
                ...
                if (existingLink && existingLink.game === LINK_SESSION_KEY) {
                    ...
                    if (
                        ...
                        log.error(`Failed to set link session for potential Player 2: ${user} in group ${groupJid}`);
                        await sock.sendMessage(user, {
            ...
            await sock.sendMessage(user, {
                text: `Kamu (P2) memilih ${this.choiceToEmoji(playerChoice)}. Menunggu player 1...`,
            });
        } else {
            await sock.sendMessage(user, {
        ...
        if (!(await sessionService.setSession(groupJid, MULTIPLAYER_SESSION_KEY, 'rps', rpsSession))) {
        ...
        if (rpsSession.player1Choice && rpsSession.player2 && rpsSession.player2Choice) {
            await this.announceMultiplayerResult(rpsSession, sock, sessionService);
        }
    }

    private async announceMultiplayerResult(session: RPSSession, sock: WebSocketInfo, sessionService: SessionService) {
        if (!session.groupJid || !session.player1Choice || !session.player2Choice || !session.player1 || !session.player2) {
            log.error('Incomplete session data for announcing multiplayer result:', session);
            if (session.groupJid) await sessionService.clearSession(session.groupJid, MULTIPLAYER_SESSION_KEY);
            if (session.player1) await sessionService.clearSession(session.player1, session.player1);
            if (session.player2) await sessionService.clearSession(session.player2, session.player2);
            return;
        }

        const result = this.determineWinner(session.player1Choice, session.player2Choice);

        let resultText = `🎮 *Hasil Rock Paper Scissors di Grup Ini*:\n\n`;
        resultText += `${this.formatUserMention(session.player1)} (P1): ${this.choiceToEmoji(session.player1Choice)}\n`;
        resultText += `${this.formatUserMention(session.player2)} (P2): ${this.choiceToEmoji(session.player2Choice)}\n\n`;
        ...
        await sessionService.clearSession(session.groupJid, MULTIPLAYER_SESSION_KEY);
    ...
    private async handleStopGame(
        ...
        const aiSession = await sessionService.getSession<RPSSession>(aiSessionKey, user);
        if (aiSession && aiSession.game === 'rps' && aiSession.data.mode === 'ai') {
        ...
        if (linkSession && linkSession.game === LINK_SESSION_KEY) {
            ...
            if (groupJid) {
                gameSession = await sessionService.getSession<RPSSession>(groupJid, MULTIPLAYER_SESSION_KEY);
            }
        } else if (isGroup) {
            gameSession = await sessionService.getSession<RPSSession>(jid, MULTIPLAYER_SESSION_KEY);
            if (gameSession && gameSession.game === 'rps' && gameSession.data.mode === 'multiplayer') {
                groupJid = jid;
            }
        }

        if (gameSession && gameSession.game === 'rps' && gameSession.data.mode === 'multiplayer' && groupJid) {
            const rpsSession = gameSession.data;
            if (user === rpsSession.player1) {
                ...
                await sock.sendMessage(groupJid, {
                    text: `🛑 Game RPS Multiplayer dihentikan oleh host (${this.formatUserMention(player1)}).`,
                    mentions: [player1],
                });
                if (!isGroup) {
                    await sock.sendMessage(user, {
    ...
    private determineWinner(choice1: NormalizedRPSChoice, choice2: NormalizedRPSChoice): 'win' | 'lose' | 'draw' {
        if (choice1 === choice2) return 'draw';
        return this.winConditions[choice1] === choice2 ? 'win' : 'lose';
    }

    private getResultText(result: 'win' | 'lose' | 'draw', player1?: string, player2?: string): string {
        switch (result) {
            case 'win':
                return player1 && player2 ? `🏆 Pemenang: ${this.formatUserMention(player1)} (P1)!` : `🎉 Kamu menang!`;
            case 'lose':
                return player1 && player2 ? `🏆 Pemenang: ${this.formatUserMention(player2)} (P2)!` : `😢 Kamu kalah!`;
            case 'draw':
                return `🤝 Seri!`;
        }
    }
    ...
    private async joinMultiplayerGame(jid: string, user: string, sock: WebSocketInfo, sessionService: SessionService) {
        const mpSession = await sessionService.getSession<RPSSession>(jid, MULTIPLAYER_SESSION_KEY);
        ...
        if (!sessionService.setSession(jid, MULTIPLAYER_SESSION_KEY, 'rps', rpsSession)) {
            await sock.sendMessage(jid, {
