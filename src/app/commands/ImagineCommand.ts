    ...
    async handleCommand(
        ...
        try {
            ...
            const imageUrl = `${this.BASE_URL}/${encodeURIComponent(prompt)}?width=${
                this.width
            }&height=${this.height}&seed=${seed}&nologo=${this.nologo}&model=${this.model}`;
            ...
            if (error instanceof AxiosError) {
                if (error.code === 'ECONNABORTED' || error.code === 'ETIMEDOUT') {
                    ...
                    errorMessage = 'woah santai dong bro! 🏃‍♀️💨\n\nAI nya lagi overwhelmed nih, tunggu 30 detik dulu deh';
                } else if (error.response?.status === 500) {
                    errorMessage = 'server AI nya lagi drama queen... emang suka lebay 💀\n\ncoba lagi nanti ya bestie';
                } else if (error.response?.status === 403) {
                    errorMessage = 'wah prompt lu kayaknya agak gimana gitu 👀\n\ncoba yang lebih family friendly dong bestie!';
                } else {
                ...
                if (error.message.includes('Empty response') || error.message.includes('Invalid image')) {
                    errorMessage =
