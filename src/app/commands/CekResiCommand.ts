...
• Data tracking real-time dari server kurir`,
  ...
  async handleCommand(
    ...
    try {
      const servicesResponse =
        await this.client.get<APIResponse<ServicesResponse>>("/");
      if (servicesResponse.data.success && servicesResponse.data.data) {
        availableServices = servicesResponse.data.data.services.map(
          (s) => s.name,
        );
      }
    } catch (error) {
  ...
  private async trackPackage(
    ...
    try {
      ...
      if (
        ...
        const fetchError = error as { response?: { data?: { error?: { message?: string } } } };
        if (fetchError.response?.data?.error) {
          throw new Error(
            fetchError.response.data.error.message || "Gagal melacak paket",
          );
        }
      }
      ...
      throw new Error(
        (error instanceof Error && error.message) ||
          `Gagal melacak paket ${courier.toUpperCase()}. Pastikan nomor resi benar.`,
      );
    }
  }
  ...
  private formatSPXResponse(data: SPXTrackingResponse): string {
    ...
    const sortedRecords = [...data.records].sort(
      (a, b) => b.actual_time - a.actual_time,
    );

    sortedRecords.forEach((record, index) => {
      const status = record.milestone_name || record.tracking_name;
      const description =
        record.buyer_description || record.description || "N/A";
