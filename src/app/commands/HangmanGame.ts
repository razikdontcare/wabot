...
async function initializeHangmanGames(sessionService: SessionService): Promise<void> {
    try {
        ...
        for (const chatJid of activeChats) {
            const sessions = await sessionService.getAllSessionsInChat<HangmanSession>(chatJid);
                        ...
                        if (
                            ...
                            (session.data.guessedLetters &&
                                (!existingData.guessedLetters ||
                                    session.data.guessedLetters.length > existingData.guessedLetters.length))
                        ) {
            ...
            for (const [gameId, gameData] of gameSessionMap.entries()) {
                if (!activeHangmanGames.has(gameId)) {
                    ...
                    const sanitizedData: HangmanSession = {
                        ...
                        hostUser:
                            gameData.hostUser ||
                            (Array.isArray(gameData.players) && gameData.players.length > 0 ? gameData.players[0] : 'unknown'),
                    };
                    ...
                    if (sanitizedData.guessedLetters.length > 0) {
                        ...
                        for (let i = 0; i < sanitizedData.word.length; i++) {
                            ...
                            newMasked += sanitizedData.guessedLetters.includes(char) ? char : MASK_CHAR;
                        }
        ...
        log.info(`Restored ${restoredGames} active Hangman games from persistent storage.`);
    } catch (error) {
...
export class HangmanGame extends CommandInterface {
    static commandInfo: CommandInfo = {
        ...
        description: 'Game tebak kata. Tebak huruf untuk menemukan kata yang tersembunyi.',
        helpText: `*Penggunaan:*
...
!hangman stop ab12cd`,
    ...
    async handleCommand(
        ...
        const userSessionLink = await sessionService.getSession<{ gameId: string }>(jid, user);
        ...
        if (command === 'start') {
            ...
            const letter = args[2];
            await this.processGameMove(letter, jid, user, gameIdToGuess, sock, sessionService);
        ...
        } else if (userSessionLink && userSessionLink.game === 'hangman' && !command) {
    ...
    private async startNewGame(jid: string, user: string, sock: WebSocketInfo, sessionService: SessionService) {
        ...
        await sock.sendMessage(jid, {
            text: `🎮 Game Hangman Multiplayer Dimulai! (ID: *${gameId}*)\n\nKata (${
                ...
                MAX_ATTEMPTS
            )}\nHint: ${definition}\n\nPemain: ${this.formatPlayerList(newSessionData)}\n\nTebak huruf dengan: ${
                BotConfig.prefix
            }hangman [huruf]\nBergabung dengan: ${BotConfig.prefix}hangman join ${gameId}`,
            mentions: [user],
        });
    }
    ...
    private async leaveGame(
        ...
        if (!userSessionLink || userSessionLink.game !== 'hangman' || userSessionLink.data.gameId !== gameId) {
            await sock.sendMessage(jid, {
    ...
    private async processGameMove(
        ...
        if (!gameData) {
            ...
            if (userSessionLink?.data.gameId === gameId) sessionService.clearSession(jid, user);
            return;
        }
        ...
        if (gameData.word.includes(letter)) {
            await this.handleCorrectGuess(jid, user, gameId, sock, gameData, sessionService);
        } else {
            await this.handleWrongGuess(jid, user, gameId, sock, gameData, sessionService);
        }
    ...
    private async handleCorrectGuess(
        ...
        let newMasked = '';
        let correctCount = 0;
        const currentGuess = gameData.guessedLetters[gameData.guessedLetters.length - 1];
        const guesserJid = user;

        for (let i = 0; i < gameData.word.length; i++) {
            const char = gameData.word[i];
            if (char === currentGuess && !gameData.maskedWord[i].includes(MASK_CHAR)) {
        ...
        gameData.playerScores[user] = (gameData.playerScores[user] || 0) + correctCount;
        gameData.maskedWord = newMasked;

        if (!newMasked.includes(MASK_CHAR)) {
            const playersByScore = Object.entries(gameData.playerScores).sort((a, b) => b[1] - a[1]);
            ...
            await sock.sendMessage(jid, {
                text: `🎉 Game *${gameId}* Selesai! Kata "${gameData.word}" berhasil ditebak oleh ${this.formatUserMention(
                    guesserJid
            ...
            for (const player of allPlayerJids) {
                const currentStat = await leaderboardService.getUserStat(player, HangmanGame.commandInfo.name);
                await leaderboardService.updateUserStat(player, HangmanGame.commandInfo.name, {
                    score: currentStat
                        ? currentStat.score
                            ? currentStat.score + gameData.playerScores[player]
                            : gameData.playerScores[player]
                        : gameData.playerScores[player],
                    wins: currentStat ? (currentStat.wins ? currentStat.wins + 1 : 1) : 1,
                });
            }
    ...
    private async handleWrongGuess(
                ...
                await leaderboardService.updateUserStat(player, HangmanGame.commandInfo.name, {
                    score: currentStat
                        ? currentStat.score
                            ...
                            : gameData.playerScores[player]
                        : gameData.playerScores[player],
                    losses: currentStat ? (currentStat.losses ? currentStat.losses + 1 : 1) : 1,
                });
            }
        ...
        await sock.sendMessage(jid, {
            text: `❌ ${this.formatUserMention(
                guesserJid
            )} menebak huruf "${currentGuess}" (salah) di game *${gameId}*. Kesempatan tersisa: ${gameData.attemptsLeft}`,
            mentions: [guesserJid],
        });
    ...
    private async updateGameStatus(jid: string, gameId: string, sock: WebSocketInfo) {
        const gameData = activeHangmanGames.get(gameId);
        if (!gameData) {
        ...
        await sock.sendMessage(jid, {
            text: [
                ...
                `Kesempatan tersisa: ${'❤️'.repeat(gameData.attemptsLeft)} (${gameData.attemptsLeft})`,
    ...
    private formatPlayerList(gameData: HangmanSession): string {
        return gameData.players.map((player) => `👤 ${this.formatUserMention(player)}`).join('\n');
    }

    private formatScoreboard(gameData: HangmanSession): string {
        if (Object.keys(gameData.playerScores).length === 0) return '_Belum ada skor_';
        return Object.entries(gameData.playerScores)
            .sort((a, b) => b[1] - a[1])
            .map(([player, score]) => `• ${this.formatUserMention(player)}: ${score} poin`)
            .join('\n');
    }
    ...
    private async endGameCleanup(
        ...
        if (deleted) {
            ...
            log.warn(`Attempted to cleanup game ${gameId}, but it was not found in activeHangmanGames.`);
        }
        ...
        for (const player of players) {
            try {
                ...
                if (playerSession && playerSession.game === 'hangman' && playerSession.data.gameId === gameId) {
                    sessionService.clearSession(jid, player);
                    log.info(`Cleared session link for player ${player} for game ${gameId}.`);
                    ...
                    log.info(`Player ${player} had no active session link found. Skipping cleanup for this player.`);
                }
            } catch (error) {
                log.error(`Error cleaning up session link for player ${player} in game ${gameId}:`, error);
            }
        }
