...
export class CooldownManager {
     ...
     */
    isOnCooldown(userId: string, command: string, cooldownMs: number, maxUses: number = 1): boolean {
     ...
     */
    getRemainingCooldown(userId: string, command: string, cooldownMs: number): number {
        const key = `${userId}:${command}`;
        const cooldownInfo = this.cooldowns.get(key);
