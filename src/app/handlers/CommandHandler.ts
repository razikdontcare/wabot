...
export class CommandHandler {
  ...
  async handleCommand(
    ...
    try {
      ...
      if (commandInfo) {
        ...
        // Check VIP status
        const { VIPService } = await import(
          "../../domain/services/VIPService.js"
        );
        ...
        const shouldCheckCooldown =
          commandInfo.cooldown && !(isVIP && vipBypassCooldown);
  ...
  async executeCommandForAI(
    ...
    try {
      ...
      const restrictedCommands = ["config", "register", "addvpn"];
  ...
  private getEffectiveUserIds(
    ...
    const keyAny = (
      msg as unknown as {
        key?: { participantAlt?: string; remoteJidAlt?: string };
      }
    ).key;
  ...
  private async isAdminConsideringAlt(
    user: string,
    msg: proto.IWebMessageInfo,
  ): Promise<boolean> {
    const config = await getCurrentConfig();
  ...
  private async getUserRolesConsideringAlt(
    user: string,
    msg: proto.IWebMessageInfo,
  ): Promise<string[]> {
    const ids = this.getEffectiveUserIds(user, msg);
      ...
      if (stats.length === 0) {
        ...
        statsText =
          ...
          stats
            .map(
              (s, i) =>
                `${i + 1}. ${s.user}: ${s.count}x (terakhir: ${s.lastUsed.toLocaleString()})`,
            )
            .join("\n");
      }
    } else {
