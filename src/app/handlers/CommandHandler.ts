import {CommandInfo, CommandInterface} from './CommandInterface.js';
import {SessionService} from '../../domain/services/SessionService.js';
import {CommandUsageService} from '../../domain/services/CommandUsageService.js';
import {getCurrentConfig, getUserRoles, log} from '../../infrastructure/config/config.js';
...
export class CommandHandler {
    ...
    async registerCommands() {
        ...
        let files: string[] = [];
        try {
            files = fs.readdirSync(commandsDir).filter((f) => f.endsWith(fileExtension));
        } catch (_error) {
            // If no files found with one extension, try the other
            const altExtension = isDev ? '.js' : '.ts';
            try {
                files = fs.readdirSync(commandsDir).filter((f) => f.endsWith(altExtension));
            } catch (_altError) {
        ...
        try {
            ...
            if (commandInfo) {
                if (commandInfo.disabled) {
                    const reason = commandInfo.disabledReason || 'Tidak ada alasan yang diberikan.';
                    await sock.sendMessage(jid, {
                ...
                if (commandInfo.cooldown) {
                    ...
                    if (this.cooldownManager.isOnCooldown(user, actualCommand, cooldownTime, maxUses)) {
                        const remainingTime = this.cooldownManager.getRemainingCooldown(user, actualCommand, cooldownTime);
                ...
                if (commandInfo.category === 'game') {
                    ...
                    await this.handleGeneralCommand(actualCommand, args, jid, user, sock, msg);
                } else if (commandInfo.category === 'admin') {
                    await this.handleAdminCommand(actualCommand, args, jid, user, sock, msg);
                } else {
                    await this.handleUtilityCommand(actualCommand, args, jid, user, sock, msg);
                }
            } else {
                await sock.sendMessage(jid, {
                    text: config.unknownCommandResponse.replace('{prefix}', config.prefix),
                });
            }
    ...
    async canUserExecuteCommand(name: string, user: string): Promise<boolean> {
        ...
        if (info.requiredRoles && info.requiredRoles.length > 0) {
            const userRoles = await getUserRoles(user);
            const hasRole = info.requiredRoles.some((role) => userRoles.includes(role));
    ...
    async executeCommandForAI(
        ...
        try {
            ...
            if (info.cooldown) {
                ...
                if (this.cooldownManager.isOnCooldown(user, commandName, cooldownTime, maxUses)) {
                    const remainingTime = this.cooldownManager.getRemainingCooldown(user, commandName, cooldownTime);
                    return {
                        success: false,
            ...
            await commandInstance.handleCommand(args, jid, user, sock, this.sessionService, msg);
    ...
    private getCommandInfo(command: string): CommandInfo | undefined {
        ...
        const actualCommand = this.aliases.has(command) ? this.aliases.get(command)! : command;
    ...
    private async extractCommand(text: string): Promise<{ command: string; args: string[] }> {
        const config = await getCurrentConfig();
    ...
    private async handleAdminCommand(
        ...
        if (!config.admins.includes(user)) {
            await sock.sendMessage(jid, {
                text: `${config.emoji.error} Kamu tidak memiliki izin untuk menggunakan perintah admin ini.`,
            });
            return;
        }
        const commandInstance = this.getCommandInstance(command);
    ...
    private async listGames(jid: string, sock: WebSocketInfo) {
        const gameCommands = Array.from(this.commands.values()).filter((cmd) => cmd.category === 'game');
        ...
        const gameList = gameCommands
            .map((game) => {
                ...
                if (game.aliases && game.aliases.length > 0) {
                    aliasText = ` (alias: ${game.aliases.map((a) => `*${config.prefix}${a}*`).join(', ')})`;
                }
    ...
    private async handleStopCommand(jid: string, user: string, sock: WebSocketInfo) {
    ...
    private async handleHelpCommand(args: string[], jid: string, sock: WebSocketInfo) {
        const config = await getCurrentConfig();
        ...
        if (args.length === 0) {
            ...
            for (const [_cmdName, info] of this.commands) {
        ...
        if (commandInfo.aliases && commandInfo.aliases.length > 0) {
            aliasText = `\n*Alias:* ${commandInfo.aliases.map((a) => config.prefix + a).join(', ')}`;
        }
    ...
    private async handleStatsCommand(jid: string, sock: WebSocketInfo, user: string, args: string[]) {
        if (!this.usageService) {
        ...
        if (args.length > 0) {
            ...
            if (stats.length === 0) {
                ...
                statsText =
                    ...
                    stats.map((s, i) => `${i + 1}. ${s.user}: ${s.count}x (terakhir: ${s.lastUsed.toLocaleString()})`).join('\n');
            }
        } else {
            // Show global stats
...
*Memory*    : ${(os.freemem() / 1024 / 1024).toFixed(2)}/${(os.totalmem() / 1024 / 1024).toFixed(2)} MB
*Load Avg*  : ${os
                    .loadavg()
                    .map((n) => n.toFixed(2))
                    .join(', ')}
