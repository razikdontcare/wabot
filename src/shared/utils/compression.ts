...
export class VideoCompressor {
  ...
  async compressVideo(inputBuffer: Buffer, options: CompressionOptions = {}): Promise<Buffer> {
    const { maxSizeMB = 50, quality = 'medium', timeout = this.DEFAULT_TIMEOUT } = options;
    ...
    try {
      ...
      const preset = quality === 'low' ? 'ultrafast' : quality === 'medium' ? 'fast' : 'medium';
      ...
      if (outputSizeMB > maxSizeMB) {
        throw new Error(`Compressed file still too large: ${outputSizeMB.toFixed(1)}MB`);
      }
