...
export class GroupSettingService {
    ...
    constructor(mongoClient: MongoClient, dbName = BotConfig.sessionName, collectionName = 'group_settings') {
    ...
    async set(group: string, data: Partial<GroupSetting>): Promise<void> {
        await this.collection.updateOne({group}, {$set: data}, {upsert: true});
    }
}
