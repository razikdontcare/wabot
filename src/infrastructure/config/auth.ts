...
export const useMongoDBAuthState = async (
    ...
    const retryOperation = async <T>(
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                log.warn(`Database operation failed (attempt ${attempt}/${maxRetries}):`, lastError.message);
    ...
    return {
        state: {
            ...
            keys: {
                ...
                set: async (data: SignalDataSet) => {
                            ...
                            for (const id in categoryData) {
                                ...
                                bulkOps.push({
                                    updateOne: {
                                        ...
                                        update: {
                                            $set: {
                                                data: value !== undefined && value !== null ? serializeData(value) : undefined,
                                            },
                                        },
                                        upsert: true,
                                    },
                    ...
                    if (bulkOps.length > 0) {
                        try {
                            await retryOperation(() => collections.keys.bulkWrite(bulkOps), 3, 800);
                        } catch (error) {
                            ...
                            for (const op of bulkOps) {
                                try {
                                    await retryOperation(
                                        () => collections.keys.updateOne(op.updateOne.filter, op.updateOne.update, {upsert: true}),
                                        2,
                                        500
                                    );
                                } catch (individualError) {
                                    log.error(`Failed to write key ${op.updateOne.filter._id}:`, individualError);
                                }
                            }
                        }
                    }
        ...
        close: async () => {
            log.info('Auth service cleanup completed (shared connection remains open)');
        },
    };
};
